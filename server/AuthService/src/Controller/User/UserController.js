...
class UserController {
    ...
    //get intructors learner
    getLearnerById = async (req, res) => {
        const { learner_id, role } = req.body;
        try {
            // Use find method to get all learners with specific IDs
            const learner = await Auth.find({ _id: learner_id, role });
            if (!learner || learner.length === 0) {
                return response(res, 404, { message: 'No learners found with the specified ID' });
            } else
                return response(res, 200, { learner });
        } catch (error) {
            console.log(error)
            return response(res, 403, { message: error.message });
        }
